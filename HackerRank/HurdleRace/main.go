...
func hurdleRace(k int32, height []int32) int32 {
	...
	var heightasInts []int

	for _, value := range height {
		heightasInts = append(heightasInts, int(value))
	}

	sort.Ints(heightasInts)
	maxHeight := heightasInts[len(heightasInts)-1]

	if maxHeight < int(k) {
		return 0
	}
	return int32(maxHeight - int(k))
}
